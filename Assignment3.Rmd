...
stargazer(ROBUST, type="text",title="Part A: Report the estimated coefficients, their heteroskedasticity-robust standard errors, and significance levels")
...
stargazer(iv_ROBUST, type="text",title="Part B: Report the IV estimates for the coefficient on happy and its heteroskedasticity-robust standard errors and significance levels")
...
# first_stage_robust
stargazer(first_stage_robust,type="text",title="Part B: Report the first stage estimates with their heteroskedasticity-robust standard errors and significance levels" )
```
...
Ans: Part C as happy should not accounting for such minor effects on the probability of having affairs as in part a and part b, though the high SE is the tradeoff we have to admit.  

3. Are Lasso/post-Lasso methods useful here for improving estimation results?

Ans: Yes. The estimates increase significantly in scale. Yet, standard error as well as wider CI are the tradeoffs.
