...
Part a.
```{r}
dgp <- function(β_one, σ){
  # Parameter
  # 1. β_one 
  # 2. σ 
  
  # Generate β0
  matrix_β_zero <- matrix(1, 30, 1)
  
  # Generate X1,1 .... X30,1
  matrix_x <- matrix(rnorm(30*1, mean=0,sd=1), 30, 1)
  
  # Generate β_one *  matrix_x
  β_one_matrix_x <-  β_one*matrix_x
  
  # Generate Ui 
  matrix_u <- matrix(rnorm(30*1, mean=0,sd=1), 30, 1)
  
  # Generate σ * U
  σ_matrix_u <- σ * matrix(rnorm(30*1, mean=0,sd=1), 30, 1)
  
  # Generate Yi
  y <- matrix_β_zero + β_one_matrix_x + σ_matrix_u
  
  # Rename matrix columns 
  colnames(y) <- "yi"
  colnames(matrix_β_zero) <- "β_zero"
  colnames(β_one_matrix_x) <- "β_one_x"
  colnames(σ_matrix_u) <- "σ_u"
  
  # cbind matrixes 
  dataset <- cbind(y,matrix_β_zero,β_one_matrix_x,σ_matrix_u)
...
dgp_mc <- function(n,β_one,σ){
  ...
  for(i in 1:n){
    ...
    # Generate dataset 
    dataset_b <- dgp(β_one,σ)
    
    # Generate lamda
    lambda_1 <- 2 * σ * ((2 * log(n * 300) / 30)**(0.5))
