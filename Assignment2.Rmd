...
PredictorVariables <- paste("V", 5:303, sep="") ## particularly construct 300 control variables 
...
#    ---2. Lasso inclduing at least one of the irrelevant regressors

# Test 8 scenarios 
# Initialize 8 counters 
for (i in 1:8){
  assign(paste("counter_",i), 0)
}

# model 1 -- Scenario 1 and 2
lasso1.model <- glmnet(X,Y,alpha=1, lambda=lambda_1)
lasso1.model$beta
lasso1.model$beta[1,1]
lasso1.model$beta[,1]
sum(lasso1.model$beta[,1]!= 0)
if (lasso1.model $beta[1,1] != 0){ # Scenario 1 -- lambda 1 and  Lasso including the first regressor Xi,1 
  counter_1<- counter_1+ 1 # Requirement satisfied and Record frequency
}

if (counter_1== 1 & sum(lasso1.model$beta != 0) > 1){  # Scenario 2 -- lambda 1 and Lasso inclduing at least one of the irrelevant regressors
...
lasso2.model $beta
sum(lasso2.model$beta != 0 )
if (lasso2.model $beta[1,1] != 0){ # Scenario 3 -- lambda 2 and  Lasso including the first regressor Xi,1 
  counter_3 <- counter_3 + 1 # Requirement satisfied and Record frequency
}

if (counter_3 == 1 & sum(lasso2.model$beta != 0) > 1){  # Scenario 4 -- lambda 2 and Lasso inclduing at least one of the irrelevant regressors
...
lasso3.model $beta
sum(lasso3.model$beta != 0 )
if (lasso3.model $beta[1,1] != 0){ # Scenario 5 -- lambda 3 and  Lasso including the first regressor Xi,1 
  counter_5 <- counter_5 + 1 # Requirement satisfied and Record frequency
}


if (counter_5 == 1 & sum(lasso3.model$beta != 0) > 1){  # Scenario 6 -- lambda 3 and Lasso inclduing at least one of the irrelevant regressors
...
lasso4.model $beta
sum(lasso4.model$beta != 0 )
if (lasso4.model $beta[1,1] != 0){ # Scenario 5 -- lambda 4 and  Lasso including the first regressor Xi,1 
  counter_7 <- counter_7 + 1 # Requirement satisfied and Record frequency
}

if (counter_7 == 1 & sum(lasso4.model$beta != 0) > 1){  # Scenario 6 -- lambda 3 and Lasso inclduing at least one of the irrelevant regressors
...
dgp_mc <- function(beta_one,sigma){
  ...
  lasso2.model <- glmnet(X, Y, alpha = 1, lambda = lambda_2)
  lasso2.model$beta
  if (lasso2.model$beta[1, 1] != 0) {
    # Scenario 3 -- lambda 2 and  Lasso including the first regressor Xi,1
    counter_3 <- 1 # Requirement satisfied and Record frequency
  }
  
  if (counter_3 == 1 &
      sum(lasso2.model$beta != 0) > 1) {
    # Scenario 4 -- lambda 2 and Lasso inclduing at least one of the irrelevant regressors
    counter_4 <- 1
  }
  ...
  if (counter_5 == 1 &
      sum(lasso3.model$beta != 0) > 1) {
    # Scenario 6 -- lambda 3 and Lasso inclduing at least one of the irrelevant regressors
    counter_6 <-  1
  }
...
Test dgp_mc function for 10 mc simulations 
```{r}
result_frame_sample <- data.frame(matrix(ncol = 8, nrow = 0))
for i in (1:10){
  dgp_mc
}
```
